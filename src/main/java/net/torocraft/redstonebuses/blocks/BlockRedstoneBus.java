...
public class BlockRedstoneBus extends BlockContainer {
  ...
  public static final PropertyEnum<BlockRedstoneBus.EnumAttachPosition> NORTH =
      PropertyEnum.create("north", BlockRedstoneBus.EnumAttachPosition.class);

  public static final PropertyEnum<BlockRedstoneBus.EnumAttachPosition> EAST =
      PropertyEnum.create("east", BlockRedstoneBus.EnumAttachPosition.class);

  public static final PropertyEnum<BlockRedstoneBus.EnumAttachPosition> SOUTH =
      PropertyEnum.create("south", BlockRedstoneBus.EnumAttachPosition.class);

  public static final PropertyEnum<BlockRedstoneBus.EnumAttachPosition> WEST =
      PropertyEnum.create("west", BlockRedstoneBus.EnumAttachPosition.class);
  ...
  private static int getAABBIndex(IBlockState state) {
    int i = 0;
    boolean flag = state.getValue(NORTH) != BlockRedstoneBus.EnumAttachPosition.NONE;
    boolean flag1 = state.getValue(EAST) != BlockRedstoneBus.EnumAttachPosition.NONE;
    boolean flag2 = state.getValue(SOUTH) != BlockRedstoneBus.EnumAttachPosition.NONE;
    boolean flag3 = state.getValue(WEST) != BlockRedstoneBus.EnumAttachPosition.NONE;
  ...
  private BlockRedstoneBus.EnumAttachPosition getAttachPosition(IBlockAccess worldIn, BlockPos pos, EnumFacing direction) {
    BlockPos blockpos = pos.offset(direction);
    IBlockState iblockstate = worldIn.getBlockState(pos.offset(direction));

    if (!canConnectTo(worldIn.getBlockState(blockpos), direction, worldIn, blockpos) && (iblockstate.isNormalCube() || !canConnectUpwardsTo(worldIn,
        blockpos.down()))) {
      IBlockState iblockstate1 = worldIn.getBlockState(pos.up());

      if (!iblockstate1.isNormalCube()) {
        boolean flag =
            worldIn.getBlockState(blockpos).isSideSolid(worldIn, blockpos, EnumFacing.UP) || worldIn.getBlockState(blockpos).getBlock() == Blocks.GLOWSTONE;

        if (flag && canConnectUpwardsTo(worldIn, blockpos.up())) {
          if (iblockstate.isBlockNormalCube()) {
            return BlockRedstoneBus.EnumAttachPosition.UP;
          }

          return BlockRedstoneBus.EnumAttachPosition.SIDE;
        }
      }

      return BlockRedstoneBus.EnumAttachPosition.NONE;
    } else {
      return BlockRedstoneBus.EnumAttachPosition.SIDE;
    }
  }
